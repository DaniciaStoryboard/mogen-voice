...
Adapted from the work of Sulun, et al. (2022), which is in turn based
on Huang, et al.(2018).
"""
import math
...
def instantiate_model(
    ...
    """
    Instantiate a model from a checkpoint path.
    """
    ...
    assert Path.exists(
        checkpoint_path / "model_config.pt"
    ), f"Config file does not exist in {checkpoint_path}."
    assert Path.exists(
        checkpoint_path / "mappings.pt"
    ), f"Mappings file does not exist in {checkpoint_path}."
    assert Path.exists(
        checkpoint_path / "model.pt"
    ), f"Model file does not exist in {checkpoint_path}."

    cfg = torch.load(checkpoint_path / "model_config.pt", map_location="cpu")
    mappings = torch.load(checkpoint_path / "mappings.pt", map_location="cpu")
    parameters = torch.load(checkpoint_path / "model.pt", map_location="cpu")

    cfg = _build_config(cfg)
...
def _build_config(config: dict) -> dict:
    return dict(
        ...
        d_condition=config["d_condition"],
        max_seq=2048,
        pad_token=0,
    )
...
def generate_mask(x, pad_token=None, batch_first=True):
    ...
    subsequent_mask = (
        torch.logical_not(torch.triu(torch.ones(seq_len, seq_len, device=x.device)).t())
        .unsqueeze(-1)
...
class MusicTransformer(torch.nn.Module):
    def __init__(
        ...
        self.pos_encoding = DynamicPositionEmbedding(
            self.embedding_dim, max_seq=max_seq
        )
...
class EncoderLayer(torch.nn.Module):
    def __init__(
        self, d_model, d_inner, rate=0.1, h=16, additional=False, max_seq=2048
    ):
        ...
        [
            [
                [
                    math.sin(
                        pos
                        * math.exp(-math.log(10000) * i / embedding_dim)
                        * math.exp(math.log(10000) / embedding_dim * (i % 2))
                        + 0.5 * math.pi * (i % 2)
                    )
...
class DynamicPositionEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        if x.device != self.device or x.dtype != self.dtype:
            self.positional_embedding = self.positional_embedding.to(
...
class RelativeGlobalAttention(torch.nn.Module):
    ...
    def _skewing(self, tensor: torch.Tensor):
        ...
        reshaped = torch.reshape(
            padded,
            shape=[padded.size(0), padded.size(1), padded.size(-1), padded.size(-2)],
        )
        Srel = reshaped[:, :, 1:, :]
    ...
    def _qe_masking(qe):
        mask = sequence_mask(
            torch.arange(qe.size()[-1] - 1, qe.size()[-1] - qe.size()[-2] - 1, -1).to(
                qe.device
            ),
            qe.size()[-1],
        )
